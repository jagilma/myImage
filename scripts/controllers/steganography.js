...
Cover.prototype.encode = function(message, image, options) {
  ...
  //console.log("IMAGE A ENCODE",imageData);
  ...
  //imageData.data = data; //Wrong. See http://stackoverflow.com/questions/11098419/imagedata-data-assignment-in-strict-mode and https://developer.mozilla.org/en-US/docs/Web/API/Canvas_API/Tutorial/Pixel_manipulation_with_canvas
...
Cover.prototype.decode = function(image, options) {
  ...
  shadowCanvas.style.display = 'none';
  shadowCanvas.width = options.width || image.width || 304;
  shadowCanvas.height = options.width || image.height || 236;
  if(options.height && options.width) {
    shadowCtx.drawImage(image, 0, 0, options.width, options.height );
  } else {
    shadowCtx.drawImage(image, 0, 0);
